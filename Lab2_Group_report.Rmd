...
We found that the first time that the atmospheric CO2 levels are predicted to be 420ppm is April 2022, and the last time is June 2033. To be more accurate, we found that the predicted CO2 values will reach 420ppm for the first time sometime in between April 2031 and May 2031 and for the last time sometime in between Jun 2033 and July 2033.  We can also see that the first time that the atmospheric CO2 levels are predicted to be 500ppm is May 2084, and the last time is October 2088 Again, to be more accurate, we found that the predicted CO2 values will reach 500ppm for the first time sometime in between April 2084 and May 2084 and the for the last time sometime in between October 2088 and November 2088 

```{r fig.width=5.25, fig.height=3.5}

#need ARIMA predictions 420: 2020 and 500: 2051
predictions2 <- model.fit %>%
  fabletools::forecast(h=1200)%>%
  hilo(level=95)%>%
  unpack_hilo("95%")
...
The ARIMA model  deviates from the actual values. The model values are similar until 2000, though the model does tend to underestimate the CO2 levels even during this time period. The model really deviates from the actual values at around 2003, as it starts to severely underestimate the CO2 levels. It appears that the keeling curve has shown some acceleration in the increase of the mean over time, and this might explain the gap between the ARIMA model performance and the true CO2 levels.
...
In 1997 we predicted that the CO2 levels would cross 420ppm at around April 2031, but from the actual data we can see that the CO2 levels cross 420ppm by February 2022, which is an almost 10 year difference.
```{r}
#Filter for 420ppm with some tolerance since these are doubles
...
We can see that the ARIMA model performs worse than the quadratic seasonal model by an order of roughly 10. On average the Quadratic Seasonal model prediction is off by about 0.79235 ppm and on average the ARIMA model is off by acout 8.6259 ppm. 
