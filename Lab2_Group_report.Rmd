...
```{r}
#attempting to display stargazer
...
model.fit<-co2_ts %>%
  model(fable::ARIMA(co2_val ~ 0 + pdq(0:10,0:2,0:10) + PDQ(0:10,0:2,0:10), ic="aic", stepwise=F, greedy=F))
...
We found that the first time that the atmospheric CO2 levels are predicted to be 420ppm is April 2022, and the last time is October 2024. To be more accurate, we found that the predicted CO2 values will reach 420ppm for the first time sometime in between April 2022 and May 2012 and for the last time sometime in between October 2024 and November 2024. We can also see that the first time that the atmospheric CO2 levels are predicted to be 500ppm is March 2051, and the last time is October 2052. Again, to be more accurate, we found that the predicted CO2 values will reach 500ppm for the first time sometime in between March 2052 and April 2051 and the for the last time sometime in between October 2052 and November 2052. 

```{r fig.width=5.25, fig.height=3.5}

#need ARIMA predictions 420: 2020 and 500: 2051
predictions2 <- quadratic_season %>%
  fabletools::forecast(h=756)
...
ppm420 <- predictions2 %>% 
  as.data.frame()%>%
  filter((point_prediction <= 422) & (point_prediction >= 419) )
#ppm420
...
predictions2%>%
  ...
  labs(title = "Predictions for Co2 levels (1998-2060)", 
       color = " ")+
  ylab("Date")+
...
p_linear <- ggplot() +
  geom_line(aes(x = prediction_lin$index, y = prediction_lin$.mean, colour = "Linear_Model")) +
...
<<<<<<< HEAD
The linear model is a fairly close match to the actual values. 
=======
The linear model deviates from the actual values. Across almost all values, the model underestimates the CO2 levels.
>>>>>>> b37aa76c72691815cf6022a8ab54acadddd3e098
...
The ARIMA model also deviates from the actual values.The model values are similar until 2000. However, the model starts to undersestimate the CO2 levels. This trend is similar to the linear model.
...
#need a formal test to evaluate the models 
#library(Metrics)

#co2 level crossing at 420ppm
co2_420 <- co2_present %>% filter(average > 419 & average <421)
print(paste('Year that crosses 420ppm:' ,  as.character(co2_420$year[1])))
print(paste('Predicted year 420ppm:', year(ppm420$by_month[1])))
```

The year that the co2 level crosses 420ppm is 2021, which is a year earlier than the predicted year of 2022. While the years are not too far off, this suggests that the actual growth rate of co2 levels are accelerating.
...
co2_present_bymonth %>%
  ...
  gg_tsdisplay(tot_CO2, plot_type="partial") + xlab( "Date") + ylab( "CO2 (ppm)")
```
...
co2_present_sa <- setNames(aggregate(co2_present$average, by=list(year(co2_present$time_index)), mean), 
                      c("year", "yr_co2"))
co2_present_nsa <- setNames(aggregate(co2_present$average, by=list(co2_present$month_index), mean), 
                      c("by_month", "tot_CO2"))
...
co2_train_nsa <- co2_present_nsa %>% 
  as_tsibble(index=by_month) %>% filter(year(by_month) <= max(year(by_month))-2) 
...
nsa.model.fit <-co2_train_nsa %>%
  model(fable::ARIMA(tot_CO2 ~ 1 + pdq(0:10,0:2,0:10) + PDQ(0:10,0:2,0:10), ic="aic", stepwise=F, greedy=F))

nsa.model.graph <- augment(nsa.model.fit)%>%
  ggplot(aes(x = by_month)) +
  geom_line(aes(y = tot_CO2, colour = "Data")) +
  geom_line(aes(y = .fitted, colour = "Fitted")) +
  labs(y = "Time",
...
nsa_prediction <- nsa.model.fit %>%
  ...
  autoplot() +
  geom_line(aes(x = co2_test_nsa$by_month, y = co2_test_nsa$tot_CO2, colour = 'Actual')) + 
  labs(x = "Time", y = 'CO2 levels',
...
co2_2122 <- co2_present_prediction %>% filter(year(by_month) == 2122)
mean(co2_2122$.mean)
