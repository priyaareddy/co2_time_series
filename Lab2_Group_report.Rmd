...
We will also generate a few ARIMA models and compare them to our linear model. To select our ARIMA model we use the BIC, the BIC selects against models with too many parameters. 

```{r}
model.fit<-co2_ts %>%
  model(fable::ARIMA(co2_val ~ 1 + pdq(0:10,0:2,0:10) + PDQ(0:10,0:2,0:10)))
model.fit$ts.model
