...
```{r}
co2_df <- data.frame(date = as.Date(time(co2)), co2 = as.numeric(co2)) %>%
...
p1 <- co2_df %>% ggplot(aes(x = time_index, y = co2)) + geom_line() + labs(title = 'CO2 over Time')
p2 <- ggAcf(co2_df$co2, type = 'correlation') + labs(title = 'ACF of CO2 Levels')
p3 <- ggAcf(co2_df$co2, type = 'partial')+ labs(title = 'PACF of CO2 Levels')
p4 <- gg_lag(co2, data = co2_df)
